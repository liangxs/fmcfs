...
extern unsigned int hdd_new_block (handle_t *handle, struct inode *inode,
				  unsigned int goal, int *errp);
extern unsigned int hdd_new_blocks (handle_t *handle, struct inode *inode,
				   unsigned int goal, unsigned long *count, int *errp);
extern void hdd_free_blocks (handle_t *handle, struct inode *inode,
			     unsigned int block, unsigned long count);
extern void hdd_free_blocks_sb (handle_t *handle, struct super_block *sb,
				unsigned int block, unsigned long count,
...
extern int hddfs_dirhash(const char *name, int len, struct
			 dx_hash_info *hinfo);
/* inode 分配相关 - ialloc.c */
extern struct inode * hdd_new_inode (handle_t *, struct inode *, int);
extern void hdd_free_inode (handle_t *, struct inode *);
...
/* 对 inode 的操作 - inode.c */
int hdd_forget(handle_t *handle, int is_metadata, struct inode *inode,
struct buffer_head *bh, unsigned int blocknr);
struct buffer_head * hdd_getblk (handle_t *, struct inode *, long, int, int *);
struct buffer_head * hdd_bread (handle_t *, struct inode *, int, int, int *);
int hdd_get_blocks_handle(handle_t *handle, struct inode *inode,
...
extern int  hdd_sync_inode (handle_t *, struct inode *);
...
/* 孤儿文件 - namei.c */
extern int hdd_orphan_add(handle_t *, struct inode *);
extern int hdd_orphan_del(handle_t *, struct inode *);
